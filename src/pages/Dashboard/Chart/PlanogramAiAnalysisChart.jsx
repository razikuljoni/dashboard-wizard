...
import PlanogramAiAnalysis from "./ExpandChart/PlanogramAiAnalysis/PlanogramAiAnalysis";

const PlanogramAiAnalysisChart = ({ accuracy, data }) => {
    const accuracyValue = Math.round(accuracy?.find(x => x._id === "Planogram")?.avg || 0);

    const ghw = data?.planogramGhwPercentage || 0;
    const vlaidSequence = data?.planogramValidSeqPercentage || 0;
    ...
    const csvData = [
        {
            "Planogram Avg. Type": "Planogram Accuracy",
            Average: accuracyValue.toFixed(2) + "%",
        },
        {
            "Planogram Avg. Type": "Planogram GHW",
            Average: ghw.toFixed(2) + "%",
        },
        {
            "Planogram Avg. Type": "Planogram Valid Sequence",
    ...
    return (
        <>
            <PlanogramAiAnalysis isModalOpen={expandedChart} setExpandedChart={setExpandedChart} />
            <div className="bg-white shadow  rounded-lg h-[285px]" id="planogram-ai-analysis">
                <ChartHeader
                    title="Planogram AI Analysis (Avg.)"
                    setExpandedChart={setExpandedChart}
                    downloadOptions={["PNG", "SVG", "CSV"]}
                    csvData={csvData}
                    cssId="planogram-ai-analysis"
...
export default PlanogramAiAnalysisChart;
