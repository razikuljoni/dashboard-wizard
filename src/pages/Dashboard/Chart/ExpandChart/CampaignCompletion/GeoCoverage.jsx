...
const GeoCoverage = ({ data }) => {
    const [chartData, setChartData] = useState([]);

    useEffect(() => {
        setChartData(data?.map(x => ({ region: x.demarcation, value: x.coverage })));
    }, [data?.length]);
    ...
    const csvData = chartData.map(({ region, value }) => ({
        Demarcation: region,
        Coverage: formatAccuracy(value),
    }));

    return (
        <div className="px-5 py-2" id="campaign-coverage-by-geography">
            <div className="p-3 bg-white rounded-lg shadow">
                <ChartHeader
                    title="Campaign Coverage By Geography"
                    cssId="campaign-coverage-by-geography"
