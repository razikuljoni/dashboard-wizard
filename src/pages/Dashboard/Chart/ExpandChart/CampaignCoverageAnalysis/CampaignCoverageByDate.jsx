...
const CampaignCoverageByDate = ({ data }) => {
    const [chartData, setChartData] = useState([]);

    useEffect(() => {
        setChartData(data?.map(x => ({ Date: x.day, Coverage: x.coverage })));
    }, [data?.length]);

    const xAxisData = chartData?.map(item => item.Date);
    const seriesData = chartData?.map(item => item.Coverage);
    ...
    return (
        <div className="px-5 py-2" id="campaign-coverage-by-date">
            <div className="p-3 bg-white rounded-lg shadow">
                <ChartHeader
                    title="Campaign Coverage By Date"
                    cssId="campaign-coverage-by-date"
...
export default CampaignCoverageByDate;
